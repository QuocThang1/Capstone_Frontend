...
const deleteBoardColumnApi = (
  projectId,
  columnName,
  targetColumnName = null,
) => {
  const URL_API = `/v1/api/projects/${projectId}/board-columns/${encodeURIComponent(
    columnName,
  )}`;
...
const deleteIssueTypeApi = (
  projectId,
  issueTypeName,
  targetTypeName = null,
) => {
  const URL_API = `/v1/api/projects/${projectId}/issue-types/${encodeURIComponent(
    issueTypeName,
  )}`;
  const data = {};
...
const createSmartProjectApi = (prompt) => {
  const URL_API = "/v1/api/projects/ai/create";
  return axios.post(URL_API, { prompt }, { timeout: 120000 }); // Tăng timeout lên 2 phút cho AI
};

const confirmSmartProjectApi = (projectId) => {
  const URL_API = `/v1/api/projects/${projectId}/ai-confirm`;
...
export {
  ...
  createSmartProjectApi,
  confirmSmartProjectApi,
};
