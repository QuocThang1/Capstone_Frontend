...
const getBottlenecksByProjectApi = (projectId, filter = {}) => {
  const URL_API = `/v1/api/bottlenecks/project/${projectId}`;
  return axios.get(URL_API, { params: filter });
};
...
const getUnresolvedBottleneckCountByUserApi = () => {
  const URL_API = "/v1/api/bottlenecks/my/unresolved-count";
  return axios.get(URL_API);
};
...
export {
  ...
  getUnresolvedBottleneckCountByUserApi,
};
